; Python highlights — копия из helix 25.07.1 runtime с одним дополнением.
;
; Штатный запрос отдаёт только `(string) @string`, поэтому докстринги были
; неотличимы от обычных строк и красились в зелёный. Блок ниже помечает
; докстринги модуля / класса / функции как `string.documentation`, а тема
; alabaster_gruvbox_material_dark_soft красит этот scope в жёлтый —
; так многострочная документация отличается от строк в кавычках.
;
; ВНИМАНИЕ: файл целиком перекрывает встроенный запрос. После обновления
; helix стоит сверить его с
; $(hx --health | grep -i runtime)/queries/python/highlights.scm

; -------
; Docstrings (добавлено)
; -------
